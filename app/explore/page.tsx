  ...
  return (
    <Link
      ...
      style={{
        ...
        transition: `opacity 0.6s ease ${delay}ms, transform 0.6s ease ${delay}ms, box-shadow 0.3s ease, ring-color 0.3s ease`,
      }}
    >
      ...
      <div className="flex items-center justify-between">
        <span className="font-ui text-[10px] tracking-wide uppercase" style={{ color }}>{VITALITY_STATUS_LABELS[lang.status]}</span>
        <span className="font-mono text-[10px] text-stone/40">{lang.vitalityScore}/100</span>
      </div>

      {/* Featured phrase snippet */}
      {lang.featuredPhrase && (
...
export default function ExplorePage() {
  ...
  // Group by continent for editorial layout
      ...
      <main className="min-h-screen bg-background pt-[72px]">
        ...
        <div className="bg-navy-abyss text-ivory relative overflow-hidden">
          <div
            ...
            style={{
              background: 'radial-gradient(ellipse 60% 80% at 60% 50%, rgba(200,169,107,0.04) 0%, transparent 60%)'
            }}
            aria-hidden="true"
          />
          <div className="relative max-w-7xl mx-auto px-6 lg:px-16 py-20 lg:py-28">
            ...
            <p className="font-body text-ivory/35 text-lg max-w-xl">
              2,847 documented languages across 147 countries. Explore by
              ...
              {CONTINENTS.map((c) => (
                <button
                  ...
                  className={`font-ui text-xs px-3.5 py-2 rounded-lg transition-all duration-300 ${
                    continent === c
                      ? 'glass-navy-heavy text-ivory shadow-sm'
                      : 'text-stone hover:text-navy glass hover:shadow-sm'
                  }`}
                >
                  {c}
                </button>
        ...
        {/* Language cards — grouped by continent with editorial narrative */}
