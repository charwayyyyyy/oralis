    ...
    return (
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-24 text-center">
        <div className="max-w-lg mx-auto">
          <div className="w-16 h-16 bg-gold/15 flex items-center justify-center mx-auto mb-6">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path d="M5 12l5 5L20 7" stroke="#C8A96B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
          ...
          <div className="inline-block border border-border p-4 text-left mb-8">
            <p className="font-ui text-xs text-stone tracking-widest uppercase mb-2">Your contribution</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button
              onClick={() => {
                setSubmitted(false); setStep(1); setSelectedLanguage('')
                setContentType(''); setTitle(''); setBody(''); setContext(''); setAudioUploaded(false)
              }}
              className="font-ui text-sm px-6 py-3 bg-navy text-ivory hover:bg-navy-deep transition-colors"
            >
              Contribute Again
            </button>
            <a
              href="/explore"
              className="font-ui text-sm px-6 py-3 border border-border text-foreground hover:bg-muted transition-colors"
            >
              Explore Languages
            </a>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <aside className="lg:col-span-4">
          <div className="lg:sticky lg:top-28">
            <p className="font-ui text-xs font-medium tracking-widest uppercase text-stone mb-6">
              ...
              {STEPS.map((s, i) => (
                <div key={s.number} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className={`w-8 h-8 flex items-center justify-center shrink-0 font-ui text-xs font-bold transition-all ${
                      step > s.number
                        ? 'bg-gold text-ink'
                        : step === s.number
                        ? 'bg-navy text-ivory'
                      ...
                      {step > s.number ? (
                        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true">
                          <path d="M2.5 6l2.5 2.5 5-5" stroke="#1A1814" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      ) : s.number}
                    </div>
                    {i < STEPS.length - 1 && (
                      <div className={`w-px flex-1 min-h-[2rem] transition-colors ${step > s.number ? 'bg-gold/50' : 'bg-border'}`} />
                    )}
                  </div>
                  <div className="pb-6 pt-1">
                    <p className={`font-ui text-sm font-medium transition-colors ${
                      step === s.number ? 'text-navy' : step > s.number ? 'text-gold' : 'text-stone'
                    }`}>
                      {s.label}
                    </p>
                    <p className="font-body text-xs text-stone/70 mt-0.5">{s.description}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-4 p-4 bg-navy/5 border border-navy/10">
              <p className="font-ui text-xs text-stone leading-relaxed">
                Your contribution will be permanently stored in our distributed AWS archive
              ...
              <div>
                ...
                <div className="space-y-2 mb-6">
                  {LANGUAGES.map((l) => (
                    ...
                    >
                      <div>
                        <span className="font-display text-base font-bold text-navy">{l.name}</span>
                        <span className="font-body text-sm text-stone ml-2">{l.nativeName}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="font-ui text-xs text-stone">{l.country}</span>
                        {selectedLanguage === l.id && (
                          <div className="w-5 h-5 bg-navy flex items-center justify-center">
                            <svg width="10" height="10" viewBox="0 0 10 10" fill="none" aria-hidden="true">
                              <path d="M2 5l2 2 4-4" stroke="#F7F4EE" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                          </div>
                        )}
                      </div>
                ...
                <button className="font-ui text-sm text-gold hover:text-navy transition-colors flex items-center gap-2">
                ...
                <div className="grid grid-cols-2 gap-3 mb-8">
                  {([
                    ...
                    >
                      ...
                      <p className="font-display text-base font-bold text-navy mb-1">{type.label}</p>
                      <p className="font-body text-xs text-stone">{type.desc}</p>
                    </button>
                  ))}
                </div>
                ...
                {contentType && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border border-border bg-background font-body text-sm focus:outline-none focus:border-navy text-foreground placeholder:text-stone/40"
                    />
                    {contentType !== 'audio' && (
                      <div className="mt-4">
                        <label className="block font-ui text-xs font-medium tracking-wide text-stone mb-2">
                        ...
                        <textarea
                          ...
                          className="w-full px-4 py-3 border border-border bg-background font-body text-sm focus:outline-none focus:border-navy text-foreground placeholder:text-stone/40 resize-none"
                        />
                      </div>
                    )}
                  </div>
              ...
              <div>
                ...
                <div
                  className={`border-2 border-dashed p-12 text-center transition-all cursor-pointer ${
                    audioUploaded ? 'border-gold bg-gold/5' : 'border-border hover:border-navy/30'
                  }`}
                ...
                >
                  {audioUploaded ? (
                    <div>
                      <div className="w-12 h-12 bg-gold/15 flex items-center justify-center mx-auto mb-3">
                ...
                <p className="mt-6 font-ui text-xs text-stone/50">
              ...
              <div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="flex items-start gap-3">
                    <input type="checkbox" id="consent" className="mt-1 accent-navy" />
                    <label htmlFor="consent" className="font-body text-sm text-stone leading-relaxed">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={label} className="flex items-start gap-4 border-b border-border pb-4">
                      <span className="font-ui text-xs text-stone w-36 shrink-0 pt-0.5 tracking-wide">{label}</span>
            ...
            <div className="flex items-center justify-between mt-10 pt-6 border-t border-border">
              ...
              {step < 5 ? (
                <button
                  ...
                  disabled={!canProceed()}
                  className="font-ui text-sm px-8 py-3 bg-navy text-ivory hover:bg-navy-deep transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  Continue &rarr;
                </button>
              ) : (
                <button
                  ...
                  className="font-ui text-sm px-8 py-3 bg-gold text-ink font-medium hover:bg-gold-pale transition-colors"
                >
                  Submit to Archive
                </button>
              )}
